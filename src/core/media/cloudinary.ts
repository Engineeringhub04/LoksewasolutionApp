...
/**
 * Uploads a local PDF (from expo-document-picker) to Cloudinary and returns the
 * hosted HTTPS URL — used by the Theory Answer Upload flow.
 *
 * Cloudinary's `image` endpoint only accepts image formats; a PDF must go
 * through the `raw` resource type instead, which stores (and serves back) the
 * file byte-for-byte with no processing. Uses its own folder so answer PDFs
 * never mix with profile photos in the Cloudinary media library.
 */
export async function uploadPdfToCloudinary(
  ...
  const { cloudName, uploadPreset } = AppConfig.media.cloudinary;

  const form = new FormData();
  form.append('file', { uri: localUri, name: fileName, type: 'application/pdf' } as unknown as Blob);
  form.append('upload_preset', uploadPreset);
  form.append('folder', 'exam-answers');
